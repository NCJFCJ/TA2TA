...
class Event_managerModelSettings extends JModelForm{
	...
	protected function populateState(){
        ...
        if (JFactory::getApplication()->input->get('layout') == 'edit') {
            $id = JFactory::getApplication()->getUserState('com_event_manager.edit.webinar.id');
        } else {
            $id = JFactory::getApplication()->input->get('id');
            JFactory::getApplication()->setUserState('com_event_manager.edit.webinar.id', $id);
        }
		$this->setState('webinar.id', $id);

		// Load the parameters.
        $params = $app->getParams();
        $params_array = $params->toArray();
        if(isset($params_array['item_id'])){
            $this->setState('webinar.id', $params_array['item_id']);
        }
	...
	public function getForm($data = array(), $loadData = true){
		// Get the form.
		$form = $this->loadForm('com_event_manager.settings', 'webinar', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData(){
		$data = JFactory::getApplication()->getUserState('com_event_manager.edit.webinar.data', array());
        if(empty($data)){
            $data = $this->getData();
        }
    ...
    $db = JFactory::getDbo();
		...
		// construct the query
		$query = 'INSERT INTO ' . $db->quoteName('#__event_manager_user_webinar');
