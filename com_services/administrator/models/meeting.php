 ...
 * Services model.
 */
class ServicesModelmeeting extends JModelAdmin{
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Meeting', $prefix = 'ServicesTable', $config = array()){
	...
	protected function loadFormData(){
		...
		if(empty($data)){
			...
			// Support for support options
			$data->support_options = '';
			if($data->id){
				$db = JFactory::getDbo();
				$query = $db->getQuery(true);
				$query->select($db->quoteName('support_option'));
				$query->from($db->quoteName('#__services_meeting_request_support_options'));
				$query->where($db->quoteName('request') . ' = ' . $data->id);
				$db->setQuery($query);
				$data->support_options = implode(',',$db->loadColumn());
			}     
		}
	...
	public function getItem($pk = null){
		if($item = parent::getItem($pk)){
			...
			if($item->id){
				...
				$query->where($db->quoteName('a.service') . ' = ' . $db->quote($item->id) . ' AND ' . $db->quoteName('a.service_type') . '=' . $db->quote('meeting'));
	...
	public function save($data){
		...
		try{
			...
			if(isset($data['support_options'])){
				// store the data locally	
				$support_options = $data['support_options'];
				
				// remove the data from the array
				unset($data['support_options']);
				
				// remove any unaccosiated records
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__services_meeting_request_support_options'));
				$query->where(array(
					$db->quoteName('request') . '=' . $db->quote($table->id),
					$db->quoteName('support_option') . 'NOT IN(' . implode(',', $support_options) . ')'
				));
				$db->setQuery($query);
				$db->query();
				
				// now, add in any new records
				$query = 'INSERT INTO ' . $db->quoteName('#__services_meeting_request_support_options');
				$query .= '(' . $db->quoteName('request') . ',' . $db->quoteName('support_option') . ') VALUES ';
				$values = array();
				foreach($support_options as $support_option){
					$values[] = '(' . $db->quote($table->id) . ',' . $db->quote($support_option) . ')';
				}
				$query .= implode(',', $values);
				$query .= ' ON DUPLICATE KEY UPDATE ' . $db->quoteName('support_option') . '=' . $db->quoteName('support_option') .';';
				$db->setQuery($query);
				$db->query();
			}else{
				// remove any unaccosiated records
			...
			if($fileUpload){
				...
				$uploadPath = JPATH_SITE . '/media/com_services/materials/meetings/' . $data['file'] . '.pdf';
				...
				if(!JFile::upload($file->tmp_name, $uploadPath)) {
					...
					$query = $db->getQuery(true);
					$query->delete($db->quoteName('#__services_meeting_requests'));
				...
				if(!empty($old_file_name)){
					...
					if($old_file_name != $data['file']){
						// delete the pdf file
						$old_file_path = JPATH_SITE . '/media/com_services/materials/meetings/' . $old_file_name . '.pdf';
