 ...
 */
class ServicesModelwebinars extends JModelList{
  ...
  protected function getListQuery(){
    ...
    $query->from($db->quoteName('#__services_webinar_requests', 'a'));
    $query->where($db->quoteName('a.parent') . ' IS NULL');

    // Join over the organization
    $query->select('tap.name AS org');
    $query->join('LEFT', '#__ta_providers AS tap ON tap.id=a.org');
    ...
    if(!empty($search)){
      if(stripos($search, 'id:') === 0){
        ...
        $query->where('(tap.name LIKE '. $search .')');
      }
    }
  ...
  public function getItems(){
    ...
    // get the first date for all series
