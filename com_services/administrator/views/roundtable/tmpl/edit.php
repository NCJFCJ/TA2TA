...
<script type="text/javascript">
 Joomla.submitbutton = function(task){
    if(task == 'roundtable.cancel'){
      Joomla.submitform(task, document.getElementById('roundtable-form'));
    }else{
      if(task != 'roundtable.cancel' && document.formvalidator.isValid(document.id('roundtable-form'))){   
        Joomla.submitform(task, document.getElementById('roundtable-form'));
      }else{
...
<form action="<?php echo JRoute::_('index.php?option=com_services&layout=edit&id=' . (int) $this->item->id); ?>" method="post" enctype="multipart/form-data" name="adminForm" id="roundtable-form" class="form-validate">
  <div id="error">
  ...
  <div class="row-fluid">
    <div class="span10 form-horizontal">
      <fieldset class="adminform">
        ...
        <div class="control-group">
          <div class="control-label"><?php echo $this->form->getLabel('proposed_locations'); ?></div>
          <div class="controls"><?php echo $this->form->getInput('proposed_locations'); ?></div>
        </div>
        <div class="control-group">
        ...
        <div class="control-group">
          <?php echo $this->form->getLabel('is_partner'); ?><br>
          <?php echo $this->form->getInput('is_partner'); ?>
        </div>
        <div class="control-group">
          <?php echo $this->form->getLabel('benefit'); ?><br>
          <?php echo $this->form->getInput('benefit'); ?>
        </div>
        <div class="control-group">
          <?php echo $this->form->getLabel('how_advance'); ?><br>
          <?php echo $this->form->getInput('how_advance'); ?>
        </div>
        <div class="control-group">
          <?php echo $this->form->getLabel('goals'); ?><br>
          <?php echo $this->form->getInput('goals'); ?>
        </div>
        <div class="control-group">
          <?php echo $this->form->getLabel('resources_needed'); ?><br>
          <?php echo $this->form->getInput('resources_needed'); ?>
        </div>
        <div class="control-group">
          <?php echo $this->form->getLabel('resources_provided'); ?><br>
          ...
          <div class="controls">
            <?php
              if(!empty($this->item->file)){
                echo 'Uploaded file: <a href="/media/com_services/materials/roundtables/' . $this->item->file . '.pdf" target="_blank">' . $this->item->file . '.pdf</a><br><br>Caution: Uploading a new file will replace the current file uploaded by the user. Only one file is supported per request.<br>';
              }
          ...
          <div class="control-group">
            ...
            <div class="controls">
              ...
              if($this->item->state == 0):
                $url = 'http' . (isset($_SERVER['HTTPS']) && !empty($_SERVER['HTTPS']) ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . '/roundtables/registration/' . $this->item->alias . '.html';
              ?>
