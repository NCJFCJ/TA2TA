 ...
 * View to edit
 */
class ServicesViewRoundtable extends JViewLegacy{
	protected $state;
	protected $item;
	protected $form;
    ...
    if(isset($this->item->checked_out)){
      ...
      $checkedOut = false;
		...
		JToolBarHelper::title(JText::_('COM_SERVICES_TITLE_ROUNDTABLE'), 'roundtable.png');

		// If not checked out, can save the item.
		if(!$checkedOut && ($canDo->get('core.edit')||($canDo->get('core.create')))){
			JToolBarHelper::apply('roundtable.apply', 'JTOOLBAR_APPLY');
			JToolBarHelper::save('roundtable.save', 'JTOOLBAR_SAVE');
		}
		if(!$checkedOut && ($canDo->get('core.create'))){
			JToolBarHelper::custom('roundtable.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
		}
		if(empty($this->item->id)){
			JToolBarHelper::cancel('roundtable.cancel', 'JTOOLBAR_CANCEL');
		}else{
			JToolBarHelper::cancel('roundtable.cancel', 'JTOOLBAR_CLOSE');
		}
	}
}