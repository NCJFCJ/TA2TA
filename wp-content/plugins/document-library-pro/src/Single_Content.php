...
class Single_Content implements Registerable, Service {
	...
	private function get_document_details_html() {
		...
		<div class="dlp-document-info">
			<div class="dlp-image-and-buttons">
				<?php 
					if ( in_array( 'thumbnail', $display_options, true ) && has_post_thumbnail() ) : 
						echo '<div class="dlp-document-info-image">';
				...
				<?php 
					if ( $document->get_download_url() ) :
						...
						echo '<div class="dlp-document-info-buttons">';
							echo $document->get_download_button( $options['link_text'], $options['link_style'], 'direct', $options['link_target'] );
							
							if ( $document->is_allowed_preview_mime_type() && $options['preview'] ) :
								Frontend_Scripts::load_preview_scripts();
								echo $document->get_preview_button( $options['preview_text'], $options['preview_style'], 'single' );
							endif;
						echo '</div>';
					endif; 
				?>
