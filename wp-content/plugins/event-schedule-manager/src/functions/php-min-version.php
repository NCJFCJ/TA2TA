...
if ( ! function_exists( 'tribe_get_php_min_version' ) ) :
	...
	function tribe_not_php_version_message() {
		...
		$last_connector = esc_html_x( ' and ', 'Plugin A "and" Plugin B', 'event-schedule-manager' );
		$many_connector = esc_html_x( ', ', 'Plugin A"," Plugin B', 'event-schedule-manager' );
		...
		'<br />' .
		esc_html__( 'To allow better control over dates, advanced security improvements and performance gain.', 'event-schedule-manager' ) .
		'<br />' .
		esc_html__( 'Contact your Host or your system administrator and ask to upgrade to the latest version of PHP.', 'event-schedule-manager' );
	}
