...
namespace TEC\Conference\Vendor\StellarWP\Installer;
...
class Button {
	...
	protected function get_activated_label() {
		...
		$text = __( 'Activated!', 'event-schedule-manager' );
	...
	protected function get_activating_label() {
		...
		$text = __( 'Activating...', 'event-schedule-manager' );
	...
	protected function get_installed_label() {
		...
		$text = __( 'Installed!', 'event-schedule-manager' );
	...
	protected function get_installing_label() {
		...
		$text = __( 'Installing...', 'event-schedule-manager' );
	...
	public function render( string $request_action, string $button_label = null, string $redirect_url = null ): void {
		...
		if ( empty( $button_label ) ) {
			if ( $request_action === 'activate' ) {
				$button_label = sprintf( __( 'Activate %s', 'event-schedule-manager' ), $this->handler->get_name() );
			} else {
				$button_label = sprintf( __( 'Install %s', 'event-schedule-manager' ), $this->handler->get_name() );
			}
		}
