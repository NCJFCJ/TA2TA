...
class Config {
	...
	public static function get_hook_prefix(): string {
		if ( ! static::$hook_prefix ) {
			throw new \RuntimeException(
				sprintf(
					__( 'You must provide a hook prefix via %1$s before creating an Installer.', 'event-schedule-manager' ),
	...
	public static function set_hook_prefix( string $hook_prefix ): void {
		if ( ! empty( static::$hook_prefix ) ) {
			throw new \RuntimeException(
				sprintf(
					__( 'The %1$s has already been called and set to %2$s.', 'event-schedule-manager' ),
					__CLASS__ . '::set_hook_prefix()',
					static::$hook_prefix
				)
			);
		...
		if ( $sanitized_prefix !== $hook_prefix ) {
			throw new \InvalidArgumentException( __( 'Hook prefix must only contain lowercase letters, numbers, "_", or "-".', 'event-schedule-manager' ) );
		}
