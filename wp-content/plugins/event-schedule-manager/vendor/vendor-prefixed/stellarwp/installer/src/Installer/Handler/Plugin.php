...
namespace TEC\Conference\Vendor\StellarWP\Installer\Handler;

use stdClass;
use TEC\Conference\Vendor\StellarWP\Installer\Button;
use TEC\Conference\Vendor\StellarWP\Installer\Config;
use TEC\Conference\Vendor\StellarWP\Installer\Contracts\Handler;
use TEC\Conference\Vendor\StellarWP\Installer\Installer;
use TEC\Conference\Vendor\StellarWP\Installer\Utils\Array_Utils;
use WP_Error;
...
class Plugin implements Handler {
	...
	public function get_error_message(): ?string {
		...
		$message     = sprintf(
			...
			__( 'There was an error and plugin could not be installed, %1$splease install manually%2$s.', 'event-schedule-manager' ),
	...
	public function handle_request() {
		...
		if ( ! check_ajax_referer( $installer->get_nonce_name(), 'nonce', false ) ) {
			$response['message'] = wpautop( __( 'Insecure request.', 'event-schedule-manager' ) );
		...
		if ( ! current_user_can( $this->get_permission() ) ) {
			wp_send_json_error(
				[
					'message' => wpautop(
						sprintf(
							__( 'Security Error, Need higher permissions to install %1$s.' , 'event-schedule-manager' ),
							$this->name
						)
					)
				]
		...
		if ( false === $success ) {
			...
			wp_send_json_success( [ 'message' => __( 'Success.', 'event-schedule-manager' ) ] );
		}
	}
