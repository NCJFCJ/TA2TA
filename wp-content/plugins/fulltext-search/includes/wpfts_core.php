...
class WPFTS_Core
{
	...
	public function set_is_settings_page()
	{
		...
		if (preg_match('~^fast\-total\-search_page_wpfts\-options\-~', $hook_suffix)) {
	...
	public function _activate_plugin($networkwide = false)
	{
		...
		if ((isset($_GET['action'])) && ($_GET['action'] == 'error_scrape')) {
			...
			// Check db
			$this->log('Plugin activation: updating DB');
			...
			if (strlen($expdt) > 0) {
				if (strtotime($expdt) < current_time('timestamp')) {
					...
					if ((count($res2) > 0) && ($res2[0]['n'] >= 10)) {
						// Found something
						$notify_text = '<p>'.sprintf(__('<b>Important Notice:</b> WP Fast Total Search plugin has detected <b>%s</b> files of the type <b>%s</b>', 'fulltext-search'), $res2[0]['n'], $pro_supported_mimes[$res2[0]['post_mime_type']]);
						if (count($res2) > 1) {
		...
		if (strlen($expdt) > 0) {
			if (strtotime($expdt) < current_time('timestamp')) {
				...
				if (PHP_INT_SIZE == 8) {
					...
					$notify_text = '<p>'.__('<b>Important Notice:</b> WP Fast Total Search plugin is optimized for x64 platforms, however your website is currently using 32-bit version of PHP.', 'fulltext-search').'</p>';
	...
	*/
	...
	public function ajax_submit_rebuild()
	{
		...
		if (($data = $jx->getData()) !== false) {
			//if (wp_verify_nonce($data['wpfts_options-nonce'], 'wpfts_options')) {
				...
				$this->_database->rebuildDBTables();
	...
	public function process_form_controlbox($jx, $data)
	{
		if (wp_verify_nonce($data['wpfts_options-nonce_controlbox'], 'wpfts_options_controlbox')) {
			...
			$this->set_option('preset_selector', $t);
