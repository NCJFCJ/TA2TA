...
class WPFTS_Output 
{
	...
	public function control_box($post = null) 
	{
		...
		$is_wpblocks = ($t && is_array($t) && isset($t['wpmainsearch_frontend']) && ($t['wpmainsearch_frontend'] === 'frontend_default')) ? 1 : 0;
			...
			<div class="card-body">
				<div class="row">
					<div class="col fixed-200 font-weight-bolder">
						<?php echo __('Enable Fast Total Search', 'fulltext-search'); ?>
					</div>
				...
				<div class="row">
					...
					<div class="col col-xl col-12 d-xl-block collapse" id="wf_hint2">
						<p class="text-secondary"><i><?php echo __('Normally, WP Fast Total Search will auto index any new post or post changes even if you disabled the previous option. Disabling this option will completely stop all plugin functions. However, you probably have to do a full index rebuild, if you activate the plugin again.', 'fulltext-search'); ?></i><br>
				...
				<div class="row">
					<div class="col fixed-200 font-weight-bolder">
						<?php echo __('Use for Blocks', 'fulltext-search'); ?>
					</div>
					<div class="col fixed-150">
						<?php
						echo WPFTS_Htmltools::makeLabelledCheckbox('wpfts_is_wpblocks', 1, __('Enabled', 'fulltext-search'), $is_wpblocks);
						?>
					</div>	
					<div class="col d-xl-none text-right">
					...
					<div class="col col-xl col-12 d-xl-block collapse" id="wf_hint8">
						<p class="text-secondary"><i><?php echo __('Some modern (block) themes are using Gutenberg "wp:query" blocks to render search results. This checkbox should be ON to let WPFTS work with them.', 'fulltext-search'); ?></i></p>
					</div>
				</div>
