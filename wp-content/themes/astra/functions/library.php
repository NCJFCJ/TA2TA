...
function edit_document_list() {
				...
				if ( ! empty( $status ) ) {
					if ( ! is_wp_error( $status ) ) {
						...
						foreach( $status as $term ) {
							$list_string .= ' Status: <em> ' . $term->name . '</em>';
						}
					}
				}
				$list_string .= '</div></td></tr>';
			}
	...
	// $list_string .= '<div class="pagination">';

	// 	$list_string .= paginate_links( array(
...
function populate_edit_library_form( $form ) {
	...
	foreach ( $form['fields'] as &$field ) {
		...
		// if( $field->label == 'Select Award(s)' ) {
			
		// 	$award_number = get_post_meta( $doc_post_id, 'award_number_for_library', true);
							
		// 	$choices = array();
		// 	$awards_list = get_terms_list(get_awards_Obj());
		// 	$inputs = array();
  		// 	$repeater_value = count($awards_list);
			 			
		// 	$input_id = 1;

  		// 	for ($i=0; $i<$repeater_value; $i++) {

		// 		//$award_key = 'award_number'. $i.'_award';
		// 		//echo '<br>'. $award_key . '<br>';
		// 		//$award_value = get_post_meta($award_number, $meta_key, true);

		// 		//echo 'Award Value : ' . $award_value . '  ' .  $i . '<br>';
		// 		// echo 'AWARD NUMBER : '. $award_number . '<br>'; 
		// 		// echo $awards_list[$i] . '<br><br>'; 
		// 		if( $award_number == $awards_list[$i] ) {
		// 			$choices[] = array( 'text' 	=> $awards_list[$i],
		// 								'value' 	=> $awards_list[$i],
		// 								'isSelected' => true
		// 								); 
		// 			$inputs[] = array(  'label' 	=> $awards_list[$i], 
		// 								'id'		=> "33.{$input_id}",
		// 								);
		// 		}   else    {
		// 			$choices[] = array( 'text' 	=> $awards_list[$i],
		...
		// 	$field->placeholder = 'Select Award Number';
		// 	$field->choices = $choices;	
		// 	$field->inputs = $inputs;
		// 	//  echo '<pre>';
		// 	//  print_r($choices);
		...
		// 	//admin options page
		// 	//$post_id = 12646;
		// 	$grant_programs_list = get_terms_list(get_grant_programs_Obj());
		// 	$choices = array();
		// 	$inputs = array();
		...
		if( $field->label == 'Target Audience(s)' ) {
			...
			//admin options page
			//$post_id = 12646;
			$target_audiences_list = get_terms_list(get_target_audiences_Obj());
			$choices = array();
			$inputs = array();
  			//$repeater_value = get_post_meta($post_id, 'target_audience', true);	 
...
///////////////////////////////////////////////////////////////////////////
add_filter( 'gform_chained_selects_input_choices_16_34_1', 'gf_edit_award_number', 10, 7 );
function gf_edit_award_number( $input_choices, $form_id, $field, $input_id, $chain_value, $value, $index ) {
	...
	$choices = array();
	// get from query string
	$doc_post_id = isset($_GET['id'])? $_GET['id'] : NULL;
	//post meta from library
	//$award = get_post_meta($doc_post_id, 'list_of_award_numbers', true);
	$award = get_post_meta($doc_post_id, 'award_number_for_library', true);
	$projects = organizations_grant_projects()[$org_post_id];
	$awards_list = [];
	if(is_array($projects)){
		foreach($projects as $project){
			$awards_list[] = $project['award_number'];
		}

		//$awards_list = get_terms_list(get_awards_Obj());
		$repeater_value =  get_post_meta($org_post_id, 'award_number_for_library', true);	 
		
		for ($i=0; $i< count($awards_list); $i++) {

			if( $award == $awards_list[$i] ) {
					$choices[] = array('text'  		=> $awards_list[$i],//get_post_meta($org_post_id, $meta_key, true), 
									'value' 		=> $awards_list[$i],//get_post_meta($org_post_id, $meta_key, true),
									'isSelected' => true
									);  
			} 	 
			else {
					$choices[] = array('text'  		=> $awards_list[$i],//get_post_meta($org_post_id, $meta_key, true), 
									'value' 		=> $meta_key,//get_post_meta($org_post_id, $meta_key, true),
									'isSelected' => false
									);  
			}  	 									
		}
	} else {
		$awards_list = $projects[0]['award_number'];
		$choices[] = array(	'text'  	=> $awards_list,//get_post_meta($org_post_id, $meta_key, true), 
							'value' => $awards_list,//get_post_meta($org_post_id, $meta_key, true),
							'isSelected' => true
									);  
	}   
...
add_filter( 'gform_chained_selects_input_choices_16_34_2', 'gf_edit_grant_programs', 10, 7 );
function gf_edit_grant_programs( $input_choices, $form_id, $field, $input_id, $chain_value, $value, $index ) {
    ...
    $selected_award = $chain_value[ "{$field->id}.1" ];
    if( ! $selected_award ) {
	...
	if(is_array($projects)){
		foreach($projects as $project){
			if($project['award_number'] == $selected_award){
	...
	// 	if($project_title == $selected_award){
...
add_action( 'gform_advancedpostcreation_post_after_creation_10', 'update_document_post_acf', 10, 4 );
function update_document_post_acf( $post_id, $feed, $entry, $form ) {
 

    $file_upload_field_id = 3; // The ID of the File Upload field.
 	$checkboxes_field_id = 14;
	...
	// Get field object.
    $field = GFAPI::get_field( $form, $checkboxes_field_id );
  
    if ( $field->label == 'Target Audiences' ) {
        // Get a comma separated list of checkboxes checked
    ...
    // Replace my_custom_field_key with your custom field meta key.
    update_post_meta( $post_id, 'target_audiences_for_library', $values );
}
